...
@WebServlet("/AvailableTimesServlet")
public class AvailableTimesServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String consultantIdStr = request.getParameter("date");
        ...
        if (consultantIdStr != null && !consultantIdStr.isEmpty()) {
            ...
            try {
                availableTimes = consutantDao.getTimesFormDate(consultantIdStr);
...
jsonResponse.append("{\"times\": [");
            for (int i = 0; i < availableTimes.size(); i++) {
                jsonResponse.append("\"").append(availableTimes.get(i)).append("\"");
